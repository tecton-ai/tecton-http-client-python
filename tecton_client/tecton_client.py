        ...
        Raises:
            BadRequestError: If the response returned from the Tecton Server is 400 Bad Request
                1. Error: Missing required join key: user_id
                    This error indicates that the request is missing one or more of the required join keys for your
                    feature service. To resolve this, ensure that the `join_key_map` in your request includes a union of
                    all join keys across all feature views in the feature service.
                2. Error: Expected key not found: [xxx] in requestContextMap
                    This error indicates that an expected key `xxx` was not found in the query's `requestContextMap` for
                    an On Demand Feature View (ODFV). Please verify that the `requestContextMap` includes a union of all
                ...
                3. Error: Unexpected JSON type for xxx in joinKeyMap. Expected int64 or null
                    This error indicates that your feature service expects a join_key `xxx` of type int64 but your
                    request contains a join_key `xxx` of type string. Please ensure that the join_key types in the
                    request match the data types for the corresponding join keys in your feature view definitions.
            ...
            ForbiddenError: If the response returned from the Tecton Server is 403 Forbidden
                1. Error: Not Authorized. Note that access control changes may take up to 60 seconds to apply.
                    The Service Account associated with your API Key does not have the necessary permissions to query
                    the feature service. Note that the Service Account needs to have at least Consumer privileges for a
                    workspace in order to query a feature service in that workspace. Please refer to the
                    [Tecton Documentation](https://docs.tecton.ai/docs/beta/reading-feature-data/reading-feature-data-
                    for-inference/reading-online-features-for-inference-using-the-http-api#creating-an-api-key-to-\
            ...
            NotFoundError: If the response returned from the Tecton Server is 404 Not Found
                1. Error: Unable to query FeatureService my_feature_service for workspace my_workspace. Newly created
                feature services may take up to 60 seconds to query. Also, ensure that the workspace is a live workspace
                ...
                online store.
                    ...
                    materialized data in the online store. Verify that all Feature Views that my_odfv depends on have
                    online materialization enabled and have successfully completed materialization jobs.
                ...
                regions, you may need to wait for DynamoDB Global Tables to finish creating a replica table.
                    This error occurs when using DynamoDB as the Online Store and indicates that Tecton was unable to
                    find or read from the DynamoDB Table for one or more Feature Views in the Feature Service. Please
                    review any permission changes in DynamoDB Tables on your end and if this issue persists, please
            ...
            ResourceExhaustedError: If the response returned from the Tecton Server is 429 Resources Exhausted
                1. Error: GetFeatures exceeded the concurrent request limit, please retry later
                    This error indicates that you have exceeded the concurrent request limit for your deployment, and
                    you need to either reduce your request rate or scale up your feature server deployment. Please refer
                    to the Scaling Documentation for more information and contact Tecton support for further assistance
                2. Error: DynamoDB throttled the request. The request rate exceeds the AWS account's throughput limit.
                    Tecton uses DynamoDB as the Online Store in OnDemand mode which autoscales the DynamoDB Tables based
                    on workloads. This error indicates that the current rate of requests exceeds the allowed throughput
                    for your AWS account and the DynamoDB Tables cannot be scaled further. Please reduce your request
                    rate or contact AWS Support to request a limit increase for your account. For more information on
                    Throughput Default Quotas please refer to the AWS Documentation
                3. Error: DynamoDB throttled the request. You may be requesting a hot key.
                    In DynamoDB, each partition on a table can serve up to 3000 Read Request Units and this error
                    indicates that the traffic to a partition exceeds this limit. Please reduce your request rate and
                    retry your request.
            ServiceUnavailableError: If the response returned from the Tecton Server is 503 Service Unavailable
                1. Error: 503 Service Temporarily Unavailable
                    This error indicates that Tecton is currently unable to process your request because either the
                    Online Store is unavailable or Tecton's feature server deployment is down. Please check the Online
                    Store availability on your end and if the error persists, please contact Tecton Support for
                    assistance.
            GatewayTimeoutError: If the response returned from the Tecton Server is 504 Gateway Timeout
                1. Error: Timed out
                    ...
                    because either the Online Store did not respond or Tecton was unable to process the response within
                    the time limit. Please verify the availability of your Online Store, and if the problem persists,
