...
class TestDataTypes:
    ...
    def test_str_value(self: Self) -> None:
        ...
        assert test_var.value[str(type_name.__class__)] == "test_string"
        assert len(test_var.value) == 1

    def test_int_value(self: Self) -> None:
        type_name = IntType()
        test_var = Value(type_name, 123)
        assert test_var.value[str(type_name.__class__)] == 123
    ...
    def test_float_value(self: Self) -> None:
        ...
        assert test_var.value[str(type_name.__class__)] == 123.456
        assert len(test_var.value) == 1

    def test_bool_value(self: Self) -> None:
        type_name = BoolType()
        test_var = Value(type_name, True)
        assert test_var.value[str(type_name.__class__)] is True
        assert len(test_var.value) == 1
    ...
    def test_array_value(self: Self) -> None:
        ...
        arraylist = test_var.value[str(type_name.__class__)]
        assert [item.value[str(item.value_type.__class__)] for item in arraylist] == ["test_string1", "test_string2"]
        assert len(test_var.value) == 1
    ...
    def test_struct_value(self: Self) -> None:
        ...
        datadict = test_var.value[str(type_name.__class__)]
        expected_dict = {"field1": "test_string", "field2": 123}
        assert {name: field.value[str(field.value_type.__class__)] for name, field in datadict.items()} == expected_dict
        assert len(test_var.value) == 1
